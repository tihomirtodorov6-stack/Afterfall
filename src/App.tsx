...
const SAVE_KEY = "afterfall-save-v1";
...
> = {
  ...
  metal: {
    name: "METAL FACTORY",
    icon: "⛏️",
    description: "Добива и преработва Metal.",
    max: 30,
  },
  ...
  warehouse: {
    name: "WAREHOUSE",
    icon: "📦",
    description: "Увеличава максималния капацитет.",
    max: 30,
  },
...
export default function App() {
  ...
  const upgradeBuilding = (
    ...
    if (
      ...
      setMessage(
        `Първо повиши Command Center до Level ${
          level + 1
        }.`
      );
      return;
    ...
    setMessage(
      `${buildingInfo[id].name} е подобрена до Level ${
        level + 1
      }.`
    );
  };
  ...
  const trainUnit = (
    ...
    const foodCost =
      info.food * multiplier;

    const metalCost =
      info.metal * multiplier;

    const fuelCost =
      info.fuel * multiplier;
  ...
  const researchTech = (
    ...
    window.setTimeout(() => {
      ...
      setMessage(
        `${researchInfo[type].name} достигна Level ${
          level + 1
        }.`
      );
    }, 5000);
